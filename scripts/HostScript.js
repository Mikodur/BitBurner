	...
	while (true) {
		if (ns.getServerMoneyAvailable(host) / ns.getServerMaxMoney(host) > .9) {
			...
			if (threads * ns.getScriptRam('/scripts/hack.script') > (ns.getServerMaxRam(host) - ns.getServerUsedRam(host))) {
				...
				if ((ns.getScriptRam('/scripts/hack.script') * threads) < availram) {
					...
					if (threads == 0) {
						 ...
						 * I don't think it gets this far down the if chain so this hasn't had a chance to break anything yet.
				...
				/** At points a threads == 0 error could sneak past here, because if threads = 0
				 * then obviously available RAM would be able to accomidate the script because it equals 0 ram usage.
				 */
